...
Dans le cas actuel de l'encodeur Impala, la représentation est globale :
...
Mais l'implémentation supporte aussi plusieurs patches latents si un futur encodeur
produit une carte spatiale \(H \times W\).
...
Elle applique une loss par horizon, puis pondère les horizons avec des poids croissants
vers le terminal.
...
Pour \(K=5\), les poids sont équivalents à :
...
Le terminal est le plus important pour le MPC, car le coût principal compare souvent
\(\hat z_{t+K}\) au but. Les horizons intermédiaires restent utiles comme supervision
auxiliaire : ils forcent le modèle à apprendre une trajectoire latente plausible et pas
seulement une association directe entre une séquence d'actions et un état final.
...
\subsection{Encodeur Impala global}

L'encodeur actuel produit une représentation globale :

\[
[B, 512, T, 1, 1]
\]

Donc le mécanisme query par patch fonctionne, mais avec un seul patch. Si l'on veut se
rapprocher davantage de V-JEPA 2, il faudra probablement utiliser un encodeur qui conserve
une grille de tokens spatiaux.
